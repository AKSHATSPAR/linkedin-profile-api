...
class LinkedInClient:
    ...
    async def fetch_profile(
        ...
        async with httpx.AsyncClient(
            ...
            result = FetchResult()
            result.documents.append(await self._fetch_primary(client, public_identifier))

            if self._settings.linkedin_fetch_section_fallbacks:
                documents, warnings = await self._fetch_sections(client, public_identifier)
    ...
    async def _fetch_primary(
        self, client: httpx.AsyncClient, public_identifier: str
    ) -> dict[str, Any]:
    ...
    async def _optional_get(
        ...
        *,
        ...
        try:
            return self._OptionalResult(document=await self._get_json(client, path, params=params))
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            if response.status_code == 404 and profile_lookup:
                raise ProfileNotFoundError("The LinkedIn profile was not found or is inaccessible")
            if response.status_code in {404, 410}:
                raise ProfileNotFoundError("This LinkedIn profile section is unavailable")
            if response.status_code == 429:
                raise UpstreamRateLimitedError("LinkedIn rate-limited the configured session")
            if response.status_code >= 500 and attempt + 1 < attempts:
                await asyncio.sleep(min(0.4 * (2**attempt), 2.0))
                continue
            if response.status_code >= 400:
                raise UpstreamResponseError(f"LinkedIn returned HTTP {response.status_code}")
