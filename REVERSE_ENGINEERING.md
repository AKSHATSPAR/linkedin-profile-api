...
used as the CSRF header. The setup helper can import a complete request header,
but it retains only allowlisted authentication, routing, locale, and session
continuity cookies. Its `li_at` and `JSESSIONID` values must match the separately
validated values. Runtime passes the structured map to HTTPX's cookie jar rather
than constructing a raw header. The client also sends the ordinary same-origin
fetch metadata and profile-page referer used by a web-client API request.
Credentials are read lazily from the environment or AWS Secrets Manager and
